...
import com.mb.android.maiboapp.rongcloud.model.Status;
import com.mb.android.maiboapp.rongcloud.model.User;
import com.mb.android.maiboapp.rongcloud.parser.GsonParser;
...
//    }
    ...
    public AbstractHttpRequest<User> getUserInfoByUserId(String userid,ApiCallback<User> callback) {
        List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>();
        nameValuePairs.add(new BasicNameValuePair("member_id", userid + ""));
        ApiReqeust<User> apiReqeust = new DefaultApiReqeust<User>(ApiReqeust.GET_METHOD, URI.create(ProjectConstants.Url.SHOW_USER_INFO),nameValuePairs, callback);
        AbstractHttpRequest<User> httpRequest = apiReqeust.obtainRequest(new GsonParser<User>(User.class), mAuthType);
        NetworkManager.getInstance().requestAsync(httpRequest);
