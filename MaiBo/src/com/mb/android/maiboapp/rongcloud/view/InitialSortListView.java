 ...
 *
 */
public class InitialSortListView extends ListView implements OnScrollListener {
	// 隐藏状态
	...
	public InitialSortListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		this.setOnScrollListener(this);// 给listview设置OnScrollListener
	}

	public InitialSortListView(Context context) {
		super(context);
		this.setOnScrollListener(this);// 给listview设置OnScrollListener
	}

	public InitialSortListView(Context context, AttributeSet attrs, int defStyle) {
	...
	public void configureHeaderView() {
		...
		case PINNED_HEADER_PUSHED_UP: {// 头标签上升状态
			mHeaderView.layout(0, mHeaderViewY, mHeaderViewWidth,
					mHeaderViewHeight + mHeaderViewY);
			mHeaderViewVisible = true;
			break;
		}
	 ...
	 * 设置头标签的显示文本和状态
	 * 
	 * @param mHeaderViewText
	 *            文本内容
	 * @param mHeaderViewstate
	 *            标签状态
	 */
	public void setHeaderViewTextAndState(String mHeaderViewText,
			int mHeaderViewstate) {
	...
	public void configurePinnedHeader(View header, int alpha) {
		...
		lSectionHeader.setTextColor(alpha << 24
				| (lSectionHeader.getCurrentTextColor() & 0x00ffffff));
	}
	...
	private void refreshView(int visibleItemCount) {
		...
		if (firstView != null) {// 第一项存在
			...
			if (firstBottom < headerHeight) {
				...
				// 透明度根据头标签升的高度来定
				mHeaderViewAlpha = 255 * (headerHeight + mHeaderViewY)
						/ headerHeight;
			} else {
				mHeaderViewY = 0;
