...
class AsyncHttpRequest implements Runnable {
    ...
    private void makeRequest() throws IOException {
        ...
        if (HttpHelper2.isEnableGzip() && request instanceof HttpPost) {
            response = executeWithGzip();
        } else {
    ...
    // region Gzip压缩控制
    private HttpResponse executeWithGzip() {
        try {
            // set header
            HttpPost httpPost = (HttpPost) request;
            httpPost.addHeader("post-gzip", "gzip");

            // 获取原始entity
            HttpEntity httpEntity = httpPost.getEntity();
            // 从entity中读取原始string内容
