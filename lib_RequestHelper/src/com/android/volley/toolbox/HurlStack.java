 ...
 */
public class HurlStack extends Stack implements HttpStack {
    ...
    public HttpResponse performRequest(Request<?> request, Map<String, String> additionalHeaders)
        ...
        // EDIT 新增if判断：请求参数错误则直接返回
        if (request.getGzipParam() != null) {
            String gzipParam = request.getGzipParam();
            byte[] body = gzipWrapPostData(gzipParam.getBytes());
            if (body == null) {
                return new BasicHttpResponse(new BasicStatusLine(new ProtocolVersion("HTTP", 1, 1), 205, ""));
            }
        }
        // 以下为原始内容
    ...
    protected HttpURLConnection createConnection(URL url, Request<?> request) throws IOException {
        // EDIT 修改createConnection，增加request参数
        HttpHost httpHost = request.getProxy();
        if (httpHost != null) {
            java.net.Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress(httpHost.getHostName(), httpHost.getPort()));
    ...
    private static void addBodyIfExists(HttpURLConnection connection, Request<?> request)
        ...
        if (body != null) {
            ...
            if (request.getGzipParam() != null) {
                String gzipParam = request.getGzipParam();
                body = gzipWrapPostData(gzipParam.getBytes());
            }
